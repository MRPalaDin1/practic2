...
def admin_requests(request):
    if request.method == 'POST':
        ...
        if request_id and new_status:
            ...
            # Если текущий статус «сделано», не меняем его
            if current_status == 'maked':
                pass
            elif current_status == 'new':
                # Если текущий статус «новая», разрешаем выбрать только «в работе» и «сделано»
                if new_status in ('process', 'maked'):
                    request_obj.status = new_status
                    request_obj.save()
            elif current_status == 'process':
                # Если текущий статус «в работе», разрешаем выбрать только «сделано»
                if new_status == 'maked':
                    request_obj.status = new_status
                    request_obj.save()
